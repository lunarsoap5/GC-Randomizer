...
namespace mod
{
	...
	void Mod::doCustomActorModel(void* mStatus_roomControl)
	{
		tp::d_com_inf_game::GameInfo* pGameInfo = &tp::d_com_inf_game::dComIfG_gameInfo;

		u32 numActors = sizeof(customActors) / sizeof(customActors[0]);

		// Count and temp array for the checks for this stage+room
		u32 actorCount = 0;
		customActor* actors = new customActors[5];  // Assume there's never more than 5 custom checks per stage+room

		// Loop through checks and place if correct room and stage
		for (u32 i = 0; i < numActors; i++)
		{
			customActor* actor = &customActors[i];

			if (0 == strcmp(pGameInfo->nextStageVars.nextStage, actor->stage) &&
				(0xFF == actor->room || pGameInfo->nextStageVars.nextRoom == actor->room) && // 0xFF = no need to check for room
				actor->requirement())
			{
		...
		{
			// Create required structs
			tp::d_stage::Actr* Actr = new tp::d_stage::Actr[actorCount];
			tp::d_stage::dzxChunkTypeInfo chunkInfo;
			strcpy(chunkInfo.tag, "ACTR");  // has to be ACTR for the function we use
			chunkInfo.numChunks = actorCount;
			chunkInfo.chunkDataPtr = Actr;

			// Populate TRES array with data
			for (u32 i = 0; i < actorCount; i++)
			{
				customActor actor = actors[i];

				if (actor.overrides != nullptr)
				{
					actor.overrides();
				}

				strcpy(Actr[i].actorName, "Obj_gb");
				Actr[i].parameters = 0xC80C0301 | (check.chestType << 20) | (check.saveFlag << 4);
				...
				Actr[i].angle = actor.Angle;

				Actr[i].item = actor.itemID;
			}
