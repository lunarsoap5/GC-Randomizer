...
namespace mod
{
	...
	bool ChestRandomizer::isLocked(item::ItemCheck* check)
	{
		...
		case item::ItemType::Story:
			if (check->itemID != items::Item::Aurus_Memo && check->itemID != items::Item::Asheis_Sketch && check->itemID != items::Item::Shadow_Crystal && check->itemID != items::Item::Master_Sword)
			{
				// Ilia quest
				result = true;
			}
